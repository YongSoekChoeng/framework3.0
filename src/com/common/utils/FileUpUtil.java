...
public class FileUpUtil {
	...
	class FileRenamePolicy implements com.oreilly.servlet.multipart.FileRenamePolicy {
		public java.io.File rename(java.io.File f) {
			...
			try {
				...
				fileNewName = (new com.common.utils.GuidUtil()).getNewGuid() + "." + fileExt;
	...
	public FileUpUtil(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			com.common.utils.FileUtil.makeDir(saveDirectory);
			...
			if (isMultiPart) {
				...
				multiRequest = new MultipartRequest(request, saveDirectory, maxPostSize, encoding, new com.common.utils.FileUpUtil.FileRenamePolicy());
