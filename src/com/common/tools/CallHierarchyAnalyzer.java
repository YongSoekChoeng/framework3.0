	...
	private class JavaFinder{
		...
		private void fillJavaList(){
			if(javaList == null){
				...
				String[] fileList = com.common.utils.FileUtil.readFileListAll(rootPath);
				String packageNm = "";
				String clzzNm = "";
				String fileConts = "";
				if(fileList != null){
					for(int i=0 ; i<fileList.length; i++){
						if(fileList[i].toLowerCase().endsWith(".java")){
							fileConts = com.common.utils.FileUtil.readFile(fileList[i]);
							fileConts = com.common.utils.StringUtil.replace(fileConts, "\r\n", "\n");
							fileConts = com.common.utils.StringUtil.replace(fileConts, "\r\n", "\n");
							fileConts = com.common.utils.StringUtil.replace(fileConts, "\n\n", "\n");
							fileConts = com.common.utils.StringUtil.replace(fileConts, "\n\n", "\n");
							fileConts = com.common.utils.StringUtil.replace(fileConts, "\t\t", "\t");
							...
							clzzNm = com.common.utils.FileUtil.getFileName(fileList[i], false);
		...
		private Report findReculsivly(Report report){
			...
			packages = com.common.utils.StringUtil.toStrArray(packageNm, ".");
			...
			while( keys.hasMoreElements() ){
				...
				fileLines = com.common.utils.StringUtil.toStrArray(fileConts, "\n");
				...
				for(int i=0; i<fileLines.length; i++){
					...
					if(fileLine.indexOf("package ") != -1){
						fileLine = com.common.utils.StringUtil.replace(fileLine, " ", "");
					...
					if( fileLine.indexOf("(") != -1 ){
						if(com.common.utils.StringUtil.isIncluded(fileLine, "public", true) || com.common.utils.StringUtil.isIncluded(fileLine, "protected", true) || com.common.utils.StringUtil.isIncluded(fileLine, "private", true)){
							parentMethodNm = com.common.utils.StringUtil.beforeWord(fileLine, "(", methodDiv); 
						}
					}
					/* 03. 메소드명 검색 끝 */

					/* 04. 클래스의 ALIAS 검색  시작 */
					if( com.common.utils.StringUtil.isIncluded(fileLine, classFullNm, true) ){
						alias = com.common.utils.StringUtil.nextWord(fileLine, classFullNm, aliasDiv); 
						classAlias.add(alias);
					}
					if(isSamePackage || isClassImported){
						if( com.common.utils.StringUtil.isIncluded(fileLine, classNm, true) ){
							alias = com.common.utils.StringUtil.nextWord(fileLine, classNm, aliasDiv); 
							classAlias.add(alias);
						}
					}
					/* 04. 클래스의 ALIAS 검색  끝 */
					...
					for(int k=0; k<classAlias.size(); k++){
						...
						isUsed = com.common.utils.StringUtil.isIncluded(fileLine, keyword, true);
						if(isUsed){
							break;
						}
					}
				...
				if(fileList != null){
					for(int i=0 ; i<fileList.length; i++){
						if(fileList[i].toLowerCase().endsWith(".jsp")){
							...
							packageNm = "";
							jspNm = com.common.utils.FileUtil.getFileName(fileList[i], false);
